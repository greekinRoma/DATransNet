...
layer_blocks = [4] * 3
channels = [8, 16, 32, 64]
net = ASKCResUNet(layer_blocks, channels).to(device)

flops, params, results = count_flops_params( net, input)
...
ModelSpeedup(net, dummy_input=input, masks_file=masks).speedup_model()
flops, params, results = count_flops_params( net, input)
print(f"Model FLOPs {flops/1e6:.2f}M, Params {params/1e6:.2f}M")
"""""
